...
# Sexo (Hombres/Mujeres): la dimensión de género más informativa. La identidad
# trans/no binarie es minoritaria y "No revelado" son, en su mayoría, personas
# cis contadas aquí. Unificamos Masculino->Hombres y Femenina->Mujeres.
sexo <- largo[dimension=="sexo", .(personas=sum(valor)), by=etiqueta]
sexo[etiqueta %in% c("Masculino"), etiqueta := "Hombres"]
sexo[etiqueta %in% c("Femenina"),  etiqueta := "Mujeres"]
sexo <- sexo[etiqueta %in% c("Hombres","Mujeres"),
             .(personas=sum(personas)), by=etiqueta]
fwrite(sexo[order(-personas)], file.path(dir_prod,"sexo.csv"), sep=";", bom=TRUE)
